...
# Manifest
# ===========================
@main.get("/{b64config}/manifest.json", summary="Manifeste Stremio")
...
@main.get("/manifest.json", summary="Manifeste Stremio (défaut)")
...
async def animesama_catalog(
    ...
    try:
        ...
        metas = await catalog_service.get_complete_catalog(
            ...
            search=search, genre=genre, config=config,
        )
        return {"metas": metas}
    except Exception as e:
...
async def animesama_meta(request: Request, id: str = Path(...), b64config: str = Path(...)) -> Dict[str, Any]:
    config_dict = validate_config(b64config)
    config = ConfigModel(**config_dict)
...
async def meta_default(request: Request, id: str = Path(...)) -> Dict[str, Any]:
    config = ConfigModel()
    meta = await metadata_service.get_complete_anime_meta(
...
async def stream_default(request: Request, episode_id: str = Path(...)) -> Dict[str, List[Dict[str, Any]]]:
    logger.log("STREAM", f"Demande de flux pour: {episode_id}")
    episode_id_formatted = episode_id.replace(".json", "")
    config = ConfigModel()
    try:
...
async def catalog_adkami_simulcasts(request: Request, b64config: str = Path(...)):
    try:
...
async def catalog_adkami_simulcasts_default(request: Request):
    try:
...
# ===========================
# Catalogues par genre Adkami — route générique (catch-all adkami_genre_*)
# ===========================
@main.get("/{b64config}/catalog/anime/{catalog_id}.json", summary="Catalogue genre Adkami")
...
@main.get("/catalog/anime/{catalog_id}.json", summary="Catalogue genre Adkami (défaut)")
async def catalog_adkami_genre_default(request: Request, catalog_id: str = Path(...)):
    if not catalog_id.startswith("adkami_genre_"):
        raise HTTPException(status_code=404, detail=f"Catalogue inconnu: {catalog_id}")
    try:
...
async def _dispatch_catalog(request, b64config, config, catalog_id: str):
    """Dispatch vers le bon handler selon l'ID du catalogue."""
    if catalog_id == "animesama_catalog":
        return await catalog_service.get_complete_catalog(
            request=request, b64config=b64config, config=config,
        )
    if catalog_id == "adkami_simulcasts":
        return await catalog_service.get_simulcasts_catalog(request, b64config, config)
...
@main.get("/health", summary="État de santé")
